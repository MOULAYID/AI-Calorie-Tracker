...
> Source de verite : `.claude/stacks/mobiles/react-native.libs.json`. Ne pas editer cette section manuellement -- utiliser `python .claude/python/sdd_admin/sync_stack_md.py --stack-id react-native`.

#### 2.4.a Librairies CORE (installees par arch en §2.2.1, toujours)

| Lib | Version | Role |
|-----|---------|------|
| expo | 52.0.20 | SDK Expo (Metro, Router, modules natifs) |
| react | 19.0.0 | UI lib |
| react-native | 0.76.5 | Runtime cross-platform |
| react-dom | 19.0.0 | Renderer (Expo Web optionnel) |
| typescript | 5.7.2 | Strict mode |
| @types/react | 19.0.2 | Types React 19 |
| expo-router | 4.0.15 | File-based routing (defaut Expo SDK 51+) |
| expo-status-bar | 2.0.0 | StatusBar control |
| expo-constants | 17.0.3 | Config Expo a runtime |
| expo-linking | 7.0.3 | Deep links + Universal Links |
| expo-splash-screen | 0.29.18 | Splash screen control |
| expo-font | 13.0.1 | Chargement fonts custom |
| expo-image | 2.0.3 | Image cache + transformations |
| expo-localization | 16.0.0 | i18n / detection locale |
| react-native-screens | 4.4.0 | Native screens (peer Expo Router) |
| react-native-safe-area-context | 5.0.0 | Safe area insets |
| react-native-gesture-handler | 2.21.2 | Gestures (peer Expo Router) |
| react-native-reanimated | 3.16.7 | Animations natives 60fps |
| nativewind | 4.1.23 | Tailwind pour RN |
| tailwindcss | 3.4.17 | Tailwind compile NativeWind (v3 obligatoire — v4 incompatible RN) |
| zustand | 5.0.2 | State manager client |
| @tanstack/react-query | 5.62.7 | Server state cache |
| react-hook-form | 7.54.2 | Forms |
| @hookform/resolvers | 3.10.0 | Adapter Zod ↔ RHF |
| zod | 3.24.1 | Validation schemas |
| @react-native-async-storage/async-storage | 2.1.0 | KV storage non sensible |
| expo-secure-store | 14.0.1 | KV storage sensible (Keychain / Keystore) |
| eslint | 9.17.0 | Linting |
| eslint-config-expo | 8.0.1 | Regles Expo-specific |

#### 2.4.b Librairies ON-DEMAND (installees si l'US declenche)

| Capability | Lib | Version | Triggers |
|---|---|---|---|
| http-client | axios | 1.7.9 | \\baxios\\b, http-client, appel.*api.*externe |
| date-utils | date-fns | 4.1.0 | dates.*format, duree, intervalle.*temps |
| date-utils | dayjs | 1.11.13 | dayjs, dates.*format (alternative legere) |
| icons | lucide-react-native | 0.469.0 | icones, icon-set, lucide |
| icons | @expo/vector-icons | 14.0.4 | icones, vector-icons, ionicons, material-icons |
...
| camera | react-native-vision-camera | 4.6.1 | vision-camera, photo.*haute.*qualite, video |
...
| maps | @rnmapbox/maps | 10.1.34 | mapbox, carte.*custom |
...
| reactive-state | jotai | 2.11.0 | jotai, atomic.*state (alternative Zustand) |
<!-- LIBS_CATALOG_END -->
