...
**SOLID + Clean Code** : voir `.claude/stacks/fullstack/blazor-server.md §1.5` (integralement applicable — pattern .NET classique).
...
Database-First strict. Identique a `.claude/stacks/backend/dotnet-minimalapi.md §8` :
...
> Source de verite : `.claude/stacks/fullstack/aspnet-mvc-razor.libs.json`. Ne pas editer cette section manuellement -- utiliser `.claude/python/sdd_admin/sync_stack_md.py --stack-id aspnet-mvc-razor`.
...
- **Mode EF Core Database-First** : identique a `.claude/stacks/backend/dotnet-minimalapi.md §8`
