...
- Précédent : `.claude/python/sdd_reverse_scripts/build_proc_us.py` (routage
  déterministe proc-reverse)
- Implémentation prévue (post-validation) :
  - `.claude/python/sdd_reverse/code_unit_complexity.py` (classifieur, nouveau)
  ...
  - `.claude/rules/reverse-engineering.md` (§ routage, additif)
