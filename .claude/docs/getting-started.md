...
Tu écris **ce que** le logiciel doit faire (FEAT spec + mockup HTML optionnel). SDD_Pro orchestre **12 agents IA** pour déterminer **comment** :
...
L'innovation clé : **orchestration Python déterministe** (48 scripts, 0-coût) + **agents LLM** (12 spécialisés) + **5 reviewers cross-file** = pipeline auditable, idempotent et reprenable.
...
| Taxonomie d'erreurs | **167 classes `[CLASS]`** cross-agent | ❌ | ❌ |
| Télémétrie | SQLite (cost cap, audit, gates) | ❌ | ❌ |
| Déterminisme | 48 scripts Python (0 token) | ❌ | ❌ |
