...
- L'agent invoque `python .claude/python/sdd_reverse_scripts/reverse_inventory.py --project {P}` qui orchestre `scan_legacy.py`, `inventory_builder.py`, `ui_unit_detector.py`, `db_schema_extractor.py` (0 token).
...
**Path** : `.claude/python/sdd_reverse/language_signatures.yml`
...
Hérite du format SDD_Pro `error-classification.md` (3 lignes disque, 1 ligne chat). À documenter dans la **nouvelle** règle `.claude/rules/reverse-engineering.md` (pas dans le fichier principal `error-classification.md`).
...
`python .claude/python/sdd_scripts/validate_readiness.py --feat 1 --json` doit retourner exit 0 :
...
### 12.1 Tests unitaires Python (`.claude/python/tests/test_sdd_reverse_*.py`)
...
`.claude/python/tests/test_sdd_reverse_e2e.py` :
...
`python .claude/python/sdd_admin/framework_smoke.py` doit rester vert après ajout du module reverse. Si nouveau gate `reverse-isolation` souhaité, à coder en **nouveau** script `.claude/python/sdd_reverse_scripts/reverse_smoke.py` (PAS d'édition de `framework_smoke.py`).
...
2. `.claude/python/sdd_reverse/language_signatures.yml`
3. `.claude/python/sdd_reverse/feat.reverse.template.md` (template FEAT isolé, ADV-9)
4. `.claude/python/sdd_reverse/scan_legacy.py` + tests
5. `.claude/python/sdd_reverse/inventory_builder.py` + tests (fingerprint full + core, ADV-1 ; lock TTL, ADV-2)
6. `.claude/python/sdd_reverse/ui_unit_detector.py` + tests
7. `.claude/python/sdd_reverse/db_schema_extractor.py` + tests
8. `.claude/python/sdd_reverse/file_locks_local.py` + `atomic_write_local.py` (helpers isolés, ADV-2)
9. `.claude/python/sdd_reverse/feat_structure_spec.py` (contrats partagés validateurs reverse, ADV-14)
10. `.claude/python/sdd_reverse/_parity_snapshots.json` (snapshots hashes helpers `sdd_lib`, ADV-16)
11. `.claude/python/sdd_reverse_scripts/reverse_inventory.py` (CLI déterministe Phase 1)
...
19. `.claude/rules/reverse-engineering.md`
20. `.claude/loader.reverse.yml`
21. `.claude/skills/starting-a-reverse-eng/SKILL.md`
22. Fixture `.claude/python/tests/fixtures/legacy-webforms-minimal/`
...
.claude/rules/**             (existants seulement ; création OK)
.claude/skills/**            (existants seulement ; création OK)
...
.claude/docs/**              (existants seulement ; création OK)
bootstrap.py
workspace/console/**
```
