...
phase: 3a
description: Phase 3a du workflow reverse — analyse technique fidèle d'UNE unité U-N (barreau bas de l'escalier reverse). Spawn agent reverse-tech-analyst (Opus 4.8). Output workspace/output/plans/{n}-{Name}.analysis.md, consommé par /sdd-reverse-stories (3b).
...
```
code source --[3a /sdd-reverse-analyze]--> output/plans/{n}-{Name}.analysis.md
            --[3b /sdd-reverse-stories]--> output/us/{n}-{m}-{Name}.md
            --[3c /sdd-reverse-feat]------> input/feats/{n}-{Name}.md
```
...
3. `.claude/python/sdd_reverse/analysis.reverse.template.md` présent (ADV-9). Sinon → ERROR `[REVERSE_TEMPLATE_MISSING]`.
4. (Mode legacy uniquement) Lock `workspace/input/feats/.alloc.lock` libre OU stale > 30 min (TTL 1800s). Sinon → ERROR `[REVERSE_LOCK_HELD]`. En mode pré-alloué, aucun lock (C5).
...
2. **Spawn unique** `Agent(reverse-tech-analyst)` avec args = `{U-N}`.
3. L'agent suit STEP 0 à 7 de `.claude/agents/reverse-tech-analyst.md`.
4. Émission ligne chat finale `[REVERSE] {U-N} → analyse 3a {n}-{Name} (...). (PROGRESS%)`.

## Sortie

```
workspace/output/plans/{n}-{Name}.analysis.md          (analyse technique legacy)
