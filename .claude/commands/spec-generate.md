# /spec-generate — Création guidée d'une spec fonctionnelle

Crée un fichier `workspace/input/specs/{n}-{Name}.md` pré-rempli en interrogeant le PO
via 2 séries de questions courtes. Le numéro `{n}` est auto-détecté.

**Usage :** `/spec-generate` ou `/spec-generate {Nom}`
...
1. Lister `workspace/input/specs/*.md` (Glob ou ls)
2. Pour chaque fichier, extraire le préfixe numérique avant le premier `-` :
   regex `^(\d+)-.*\.md$`
3. `{n}` = max des numéros trouvés + 1. Si aucun fichier : `{n} = 1`
4. Le fichier final sera : `workspace/input/specs/{n}-{Name}.md` (ex. `1-Auth.md`, `2-Crud.md`)
...
```
Pour cadrer la spec "{Name}" (numérotée {n}), j'ai besoin de comprendre l'essentiel :
...
2. Y a-t-il des dépendances avec des specs déjà créées ? (lister les specs existantes
   trouvées dans `workspace/input/specs/` si pertinent)
3. Qu'est-ce qui est explicitement HORS scope pour cette feature ?
```

Adapter les questions au contexte (ne pas demander la dépendance si aucune spec n'existe).

Attendre la réponse avant de continuer.

---

## STEP 5 — Proposer un plan de spec

Avant de générer le fichier, présenter un résumé pour validation :

```
Voici le plan de spec pour "{n}-{Name}" :
...
Créer `workspace/input/specs/{n}-{Name}.md` à partir du template
`.claude/templates/spec.template.md`. Remplir toutes les sections avec
...
```
ERROR: /spec-generate — numérotation SFD invalide
...
manuellement ou relancer `/spec-generate` après suppression).
...
Vérifier l'existence de `workspace/output/context/constitution.md`.

### 7.5.1 Si la constitution n'existe PAS (premier `/spec-generate` du projet)
...
3. Remplir le template :
   ...
   - §3 Acteurs : extraire de la section `## Actors` de la SPEC
     créée à l'étape STEP 6. Une ligne par acteur, colonne SPECs =
     `{n}-{Name}`
...
4. Write `workspace/output/context/constitution.md` (mode `create`).

### 7.5.2 Si la constitution existe DÉJÀ (SPEC additionnelle)

Append-only sur §3 Acteurs (mêmes règles que l'agent PO §8.5.1) :
- Pour chaque acteur de la SPEC créée, append en §3 ou MAJ la colonne
  "SPECs concernées".
...
```
Spec créée : workspace/input/specs/{n}-{Name}.md
Constitution : workspace/output/context/constitution.md ({"créée" si bootstrap, "étendue" sinon})
...
- **Hors scope** : générer du code, créer des tâches techniques, lire le stack, lire les maquettes UI.
  Cette commande produit uniquement `workspace/input/specs/{n}-{Name}.md`.
