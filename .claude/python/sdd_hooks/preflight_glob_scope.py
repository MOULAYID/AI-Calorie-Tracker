...
Glob `workspace/src/**/*` during a 1.8M-token / $35 reviewer session.
The spec-compliance + code-reviewer + security-reviewer + arch-reviewer
agents all carry "anti-pattern strict : pas de Glob workspace/src/**/*"
...
While allowing :
  - scoped globs (``workspace/src/{Project}/Services/**/*.cs``)
  - globs with explicit file extension (``workspace/src/**/*.cs``,
    ``workspace/src/**/*.ts``, ...) when caller adds suffix
  - any Glob outside ``workspace/src/`` (other dirs are typically
...
_BROAD_GLOB_PATTERNS: tuple[re.Pattern, ...] = (
    # workspace/src/**/* (no extension constraint)
    re.compile(r"^workspace/src/\*\*/\*$"),
    re.compile(r"^workspace/src/\*\*$"),
    # naked **/* or **
    re.compile(r"^\*\*/\*$"),
    re.compile(r"^\*\*$"),
...
def main() -> int:
    ...
    if mode == "strict":
        ...
        warn(f"CAUSE: [GLOB_SCOPE_TOO_BROAD] unbounded Glob under workspace/src/ "
             f"would risk reading 1000s of files (token explosion documented in "
             f"agents/spec-compliance-reviewer.md post-mortem)")
        warn(f"FIX: scope the pattern (e.g. 'workspace/src/{{Project}}/Services/**/*.cs') "
    ...
    warn(f"WARN: glob-scope — broad pattern '{pattern}' detected "
         f"(would risk token explosion under workspace/src/). "
