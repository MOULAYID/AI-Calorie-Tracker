...
def _check_node(project_dir: Path) -> list[tuple[str, bool, str]]:
    ...
    # Audit 2026-06-06 D3 — smoke browser + E2E Playwright skeleton.
    # The full implementation (npm run dev background + curl + timeout) requires
    # ProcessGroup management + port detection that's too brittle here. Instead :
    # detect the PRESENCE of the required scripts (`dev`, `e2e`, `e2e:install`)
    # in package.json and report `present`/`missing`. The actual smoke run is
    # delegated to CI runners (`.github/workflows/quality.yml` generated by arch
    # when CiTemplatesGeneration: true).
    # This bridges the §C.2 promise from "0% honoured" to "structural compliance
    # checked, runtime delegated to CI".
    ...
    if _is_ui_project(scripts, pkg):
        if has_dev:
            results.append(("smoke-script-present", True, f"script 'dev' detected"))
        else:
