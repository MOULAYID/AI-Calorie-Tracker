...
if str(_REPO_ROOT / ".claude" / "python") not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT / ".claude" / "python"))
from sdd_lib.exit_codes import FAIL_FAST, SUCCESS  # noqa: E402
_SRC = _REPO_ROOT / ".claude" / "rules" / "error-classification.md"
...
def _render(agent: str, sections: dict[str, str]) -> str:
    ...
    out.append(
        ...
        "`python .claude/python/sdd_admin/sync_error_class_digests.py`.\n>\n"
    ...
    if args.check:
        if stale:
            sys.stderr.write(
                ...
                "FIX: python .claude/python/sdd_admin/sync_error_class_digests.py\n"
            )
            return FAIL_FAST
