...
model: claude-sonnet-4-6   # Reformulation bornée (spec structurée → prose humaine) : périmètre Sonnet. Pas de lecture de code, pas de raisonnement d'architecture.
...
1. Charger `@.claude/rules/output-protocol.md` (chat 1L `[SPECBOOK] … (X%)`)
   et `@.claude/rules/error-classification.md` (bloc ERROR 3L `[CLASS]`).
...
5. Calculer le hash de contenu de la FEAT (staleness du cache) :
   `python .claude/python/sdd_scripts/generate_specbook.py --print-hash <chemin-feat>`.
