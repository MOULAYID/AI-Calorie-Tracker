...
2. Lire `.claude/python/sdd_reverse/us.proc.reverse.template.md`. Absent → STOP
   `[REVERSE_TEMPLATE_MISSING]` (pas de fallback inline).
3. Lire `.claude/python/sdd_reverse/language_signatures.yml` pour le `confidence_cap`
...
Voir `.claude/rules/reverse-engineering.md` §1-§6 + `.claude/docs/reverse-proc-engineering.audit.md`.
