...
model: claude-opus-4-8   # Synthèse métier d'un module (démotion plomberie, narratif transverse) = tâche Opus, comme 3c reverse-feat-composer. Défaut du pipeline = build_proc_feats.py (0 token) ; cet agent est opt-in (SDD_REVERSE_FEAT_LLM=1).
...
1. Charger `@.claude/rules/output-protocol.md` (chat 1L `[REVERSE] … (X%)`),
   `@.claude/rules/error-classification.md` (ERROR 3L `[CLASS]`),
   `@.claude/rules/reverse-engineering.md §6/§10` (taxonomie + pont /sdd-full).
...
1. Gate déterministe : `python .claude/python/sdd_reverse_scripts/validate_reverse_feat.py {n}`.
