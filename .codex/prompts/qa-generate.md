<!-- GENERATED FROM .sdd/ (commande /qa-generate) — DO NOT EDIT -->
...
if [ "$mode" = "api-tests" ]; then
  python .sdd/python/sdd_scripts/ingest_agent_report.py --type api-tests --feat {n}
fi
```
...
```bash
STATS=$(python .sdd/python/sdd_scripts/query_console_db.py feat-stats --feat {n})
```
...
- **Blocage tests rouges sur `/sdd-full`** (depuis v7.0.0, default `QaFailOnSddFull: true`) :
  ...
  - Détail rationale + format ERROR : `.sdd/rules/error-classification.md` `[QA_FAIL_BLOCKING_SDD_FULL]`.
