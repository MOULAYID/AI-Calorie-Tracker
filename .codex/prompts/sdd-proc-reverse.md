...
`/sdd-proc-reverse-full`. La procédure devient **une User Story** ; elle est
rattachée à la **FEAT de son module** (créée si absente, sinon mise à jour).

```
/sdd-proc-reverse dbo.usp_Contact_Insert
...
Identiques à `/sdd-proc-reverse-full` : `SELECT` catalogue + `OBJECT_DEFINITION`
...
1. `python .claude/python/sdd_reverse_scripts/reverse_proc_introspect.py --proc {proc-name} [--project DB]`
...
2. **Routage par complexité (0 token)** :
   `python .claude/python/sdd_reverse_scripts/build_proc_us.py --project DB --unit {U-N} --json`
...
3. `python .claude/python/sdd_reverse_scripts/build_proc_feats.py --project DB --unit {U-N}`
   → assemble/rafraîchit la FEAT du module (remontée depuis les US, jamais relecture du corps).
4. `validate_reverse_feat.py` + ligne chat `[REVERSE] {proc} → US {n}-{m}, module {Module}. (100%)`.
...
Voir `.claude/docs/reverse-proc-engineering.audit.md` + `.claude/rules/reverse-engineering.md`.
