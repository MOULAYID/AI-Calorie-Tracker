...
1. **Phase 1 (déterministe, 0 token)** :
   `python .claude/python/sdd_reverse_scripts/reverse_proc_introspect.py --full [--project DB]`
...
2. **Routage par complexité (0 token, token-efficient)** :
   `python .claude/python/sdd_reverse_scripts/build_proc_us.py --project DB --all --json`
...
5. **rung 2 (composition des FEAT modules)** :
   - **Défaut (déterministe, 0 token)** :
     `python .claude/python/sdd_reverse_scripts/build_proc_feats.py --project DB --all`
