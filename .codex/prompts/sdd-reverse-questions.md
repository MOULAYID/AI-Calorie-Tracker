...
2. **Liste** les questions ouvertes déterministiquement (jamais de parsing LLM
   du markdown) :
   `python .claude/python/sdd_reverse_scripts/reverse_questions_io.py workspace/old/{P}/.sys/questions.md --list-open --json`
...
Voir `.claude/rules/reverse-engineering.md §6` ([REVERSE_QUESTIONS_PENDING], [REVERSE_ANSWER_INGEST_FAILED]) + §10 + `.claude/agents/reverse-clarifier.md`.
