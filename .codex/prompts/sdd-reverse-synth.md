<!-- GENERATED FROM .sdd/ (commande /sdd-reverse-synth) — DO NOT EDIT -->
<!-- Phase 3.7 — couche de synthèse système (vue au-dessus de l'escalier 3a→3b→3c). Rend C4 (contexte/conteneurs/composants), ERD complet et soul.md à partir des SEULS artefacts déterministes (inventory.json, deps-graph.json, db-schema.merged.json). Lecture seule, aucun agent spawné, écrit uniquement sous .sys/synthesis/ (jamais feats/). Déterministe (0 token), non-cassant pour le pipeline existant. -->
...
```bash
python .claude/python/sdd_reverse_scripts/reverse_synth.py --project workspace/old/{LegacyProject} [--doc-level ...] [--only ...] [--json]
```
...
Voir `.claude/docs/reverse-engineering-workflow.md` (couche de synthèse, Phase 3.7).
