<!-- GENERATED FROM .sdd/ (commande /feat-generate) — DO NOT EDIT -->
<!-- /feat-generate — Création guidée d'une FEAT fonctionnelle -->
<!-- Arguments SDD passés via $ARGUMENTS (ex. numéro de FEAT). Sous-agents SDD = émulés par spawn_agent.py (wrapper Codex, Phase 3+). -->
...
1. **Test existence** : `workspace/stack/stack.md` existe et fait
   ≥ 100 octets.
...
2. Y a-t-il des dépendances avec des FEATs déjà créées ? (lister les FEATs existantes
   trouvées dans `workspace/feats/` si pertinent)
...
Créer `workspace/feats/{n}-{Name}.md` à partir du template
...
Vérifier l'existence de `workspace/.sys/.context/constitution.md`.
...
> répertoire parent `workspace/.sys/.context/` AVANT d'écrire,
...
0. **mkdir parent** (idempotent, cross-platform) :
   ```bash
   python -c "from pathlib import Path; Path('workspace/.sys/.context').mkdir(parents=True, exist_ok=True)"
   ```
1. Read `.claude/templates/constitution.template.md`.
2. Lire `workspace/stack/stack.md` (si présent) pour récupérer
...
4. Write `workspace/.sys/.context/constitution.md` (mode `create`).
...
```
FEAT créée : workspace/feats/{n}-{Name}.md
Constitution : workspace/.sys/.context/constitution.md ({"créée" si bootstrap, "étendue" sinon})

Prochaines étapes :
1. Relire le fichier et l'ajuster si besoin
2. Déposer les mockups HTML statiques dans workspace/ui/ si la feature a une UI (optionnel — convention {n}-{m}-{Name}.html)
...
- **Hors scope** : générer du code, créer des tâches techniques, lire le stack, lire les maquettes UI.
  Cette commande produit uniquement `workspace/feats/{n}-{Name}.md`.
