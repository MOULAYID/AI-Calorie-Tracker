...
<!-- Harness: codex — protection_level: B (.sdd/capability-matrix.yml). at_include: unsupported — pas de lazy-load @file en memoire : les refs @-includes du corps sont reecrites vers .sdd/... ; les charger explicitement (Read) avant usage. -->
...
> Substance déléguée à `.sdd/docs/` et `.claude/rules/`.
