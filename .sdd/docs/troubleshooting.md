...
python .sdd/python/sdd_scripts/query_console_db.py coverage --feat 1 --format md
...
python .sdd/python/sdd_scripts/query_console_db.py feat-stats --feat 1 --format md
...
# Edit .sdd/stacks/backend/{active}.libs.json
# Add the lib in `onDemand[]` with capability + triggers regex
# 2. Regenerate the .md §2.4
python .sdd/python/sdd_admin/sync_stack_md.py --stack-id dotnet-minimalapi
# 3. Re-run
/dev-backend 1-2
```
...
```bash
# Manual resolution
python .sdd/python/sdd_scripts/resolve_us_hash_sentinel.py --auto-detect
```
...
```bash
python .sdd/python/sdd_scripts/query_console_db.py security --feat 1 --format md
```
...
```bash
python .sdd/python/sdd_scripts/sdd_state.py show-run --run-id <id>
```
...
```bash
head -10 .sdd/stacks/backend/dotnet-minimalapi.md
...
Tests :
```bash
python -m pytest .sdd/python/tests/test_ownership_matrix_sync.py
python -m pytest .sdd/python/tests/test_preflight_agent_budget.py
```
...
# Path: .sdd/stacks/{category}/{stack-id}.md
# 2. Add the .libs.json catalog
# Path: .sdd/stacks/{category}/{stack-id}.libs.json
# 3. Validate
python .sdd/python/sdd_admin/validate_libs_catalog.py --stack-id your-stack
# 4. Sync §2.4
python .sdd/python/sdd_admin/sync_stack_md.py --stack-id your-stack
```
...
python .sdd/python/sdd_scripts/sdd_state.py list-runs --limit 5
```
...
catalog PR to add your combo to `.sdd/stacks/`.
...
- **Framework smoke** : `python .sdd/python/sdd_admin/framework_smoke.py` — quick health check.
