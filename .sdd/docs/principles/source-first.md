...
| **Stack MD** (`.sdd/stacks/{cat}/{stack}.md`) | Le bug touche un pattern réutilisable cross-projet | Pattern CORS Spring → §5.2.7 avec format ERROR + anti-récap grep |
...
| **Rule MD** (`.sdd/rules/{rule}.md`) | Le bug viole une règle non encore formalisée | Étendre la matrice ownership, ajouter anti-pattern |
