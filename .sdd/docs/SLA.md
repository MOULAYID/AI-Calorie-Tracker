...
Voir `@.sdd/docs/validated-combos.md §1.2` pour la liste détaillée.
...
- Source machine-readable : [`.sdd/templates/combos.json`](../templates/combos.json) ; rapport humain : [`workspace/.sys/.bench/BENCH-GLOBAL-REPORT.md`](../../workspace/.sys/.bench/BENCH-GLOBAL-REPORT.md).
...
Cf. `@.sdd/docs/VERSIONING.md` (SSoT).
...
- Test suite `python -m pytest .sdd/python/tests/` doit passer (~700 cas).
...
Via `console.db` local + `python .sdd/python/sdd_scripts/report_roi.py` :
...
- `@.sdd/docs/VERSIONING.md` — politique de version (SSoT)
- `@.sdd/docs/validated-combos.md` — détail combos Tier 1 + Tier 2
- `@.sdd/docs/COMPLIANCE.md` — RGPD, sécurité, audit trail
- `@.sdd/docs/KNOWN-LIMITATIONS.md` — limites techniques connues
- `@.sdd/docs/WHY-SDD-PRO.md` — positionnement vs concurrents
