...
`/sdd-clear` a été retiré en v6.1 (cf. `@.sdd/docs/CHANGELOG.md` ligne 1208) :
...
Deux scripts complémentaires sous `.sdd/python/sdd_admin/` (mainteneur-invoqués manuellement, déplacés depuis `sdd_scripts/` audit 2026-06-06 M3 — usage pattern manuel hors pipeline runtime) :

### 4.1 `audit_orphans.py` — détection read-only

```bash
python .sdd/python/sdd_admin/audit_orphans.py [--feat N] [--json]
```
...
```bash
python .sdd/python/sdd_admin/cleanup_orphans.py [--feat N] [--dry-run] [--yes]
```
...
1. Avant la première utilisation, lancer :
   ```
   python .sdd/python/sdd_admin/audit_orphans.py --feat all
   ```
...
- [`@.sdd/docs/CHANGELOG.md`](./CHANGELOG.md) ligne 1208 — décision de retrait `/sdd-clear` v6.1
...
- [`@.sdd/python/sdd_scripts/detect_capabilities.py`](.sdd/python/sdd_scripts/detect_capabilities.py) — base pour §6 (détection capabilities orphelines)
- [`@.sdd/python/sdd_scripts/scan_repo.py`](.sdd/python/sdd_scripts/scan_repo.py) — pattern de scan existant à réutiliser
