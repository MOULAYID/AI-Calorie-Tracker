...
- Suppression ou renommage d'une règle `.sdd/rules/*.md` référencée par
...
`@.sdd/docs/MIGRATION.md` avec script ou recette de migration.
...
- Nouveau stack dans `.sdd/stacks/{cat}/`
...
- Aucun bump MAJOR ni MINOR n'a été tagué sur `main` entre le 2026-05-19
  et le 2026-06-18.
- Aucun fichier supprimé sous `.claude/agents/`, `.sdd/rules/`,
  `.claude/commands/`, `.sdd/stacks/` sur `main` pendant la fenêtre.
...
>
> Politique complète : `@.sdd/docs/VERSIONING.md`.
...
- `@.sdd/docs/CHANGELOG.md` — historique versions (à compléter rétroactivement pour combler le gap v6.9 → v6.10.4)
- `@.sdd/docs/MIGRATION.md` — guide migration entre versions majeures
- `@.sdd/rules/ownership.md` Partie B §4 — format ADR (la gouvernance MAJOR/MINOR adopte le même format ; fusionné depuis `constitution.md` au merge v7.0.0)
