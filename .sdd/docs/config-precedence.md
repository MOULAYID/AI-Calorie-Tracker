...
> Document chargé **à la demande** (`Read @.sdd/docs/config-precedence.md`).
...
`.sdd/stacks/{cat}/{stack-id}.libs.json` — un par stack. Schéma :
`.sdd/templates/libs-catalog.schema.json` (draft 2020-12).
...
│                                                                     │
│  python .sdd/python/sdd_scripts/phase_planner.py \               │
...
| Vérification | Source |
|---|---|
| Lib présente dans `core[]` du `.libs.json` ? | `.sdd/stacks/{cat}/{id}.libs.json` |
...
python .sdd/python/sdd_scripts/preflight.py --json 2>&1 | Tee-Object workspace/.sys/preflight.json
...
- `@.sdd/python/sdd_lib/layered_config.py` — implémentation precedence #1-#4
