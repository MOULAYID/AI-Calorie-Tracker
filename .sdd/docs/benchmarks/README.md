...
- **Console.db propre** : `python .sdd/python/sdd_scripts/init_console_db.py --reset`
...
Les 3 FEATs de bench sont **figées** dans `.sdd/templates/bench-feats/` :
...
python .sdd/python/sdd_scripts/bench_run.py --snapshot-before --bench-id {id}
...
Cf. `.sdd/python/sdd_scripts/bench_run.py` (cf. §10 ci-dessous pour spec).
...
- `@.sdd/docs/roi-baseline.md` — destination des résultats consolidés
- `@.sdd/docs/validated-combos.md` — détail combos C1/C2
- `@.sdd/python/sdd_scripts/bench_run.py` — script d'agrégation
- `@.sdd/python/sdd_scripts/report_token_usage.py` — agrégation tokens existante (réutilisée par bench_run)
- `@.sdd/python/sdd_scripts/query_console_db.py` — read queries SQL
