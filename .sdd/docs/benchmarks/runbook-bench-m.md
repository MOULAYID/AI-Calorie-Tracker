...
>
> Détail complet : `@.sdd/docs/roi-baseline.md` callout "Périmètre de
> mesure (anti-R2)".
...
# python .sdd/python/sdd_scripts/init_console_db.py --reset
```
...
```python
# .sdd/docs/benchmarks/aggregate.py (1-shot, à exécuter)
import json, statistics
from pathlib import Path

runs = []
for i in (1, 2, 3):
    p = Path(f".sdd/docs/benchmarks/runs/bench-m-kotlin-run-{i}.json")
...
Créer `.sdd/docs/benchmarks/feat-m-kotlin.md` selon le template
...
# - .sdd/docs/benchmarks/runs/*.json (rapports)
# - .sdd/docs/benchmarks/feat-m-kotlin.md (synthèse)
...
- `@.sdd/templates/bench-feats/feat-m.template.md` — FEAT canonique
- `@.sdd/templates/bench-feats/mockups/feat-m-1-orders-list.html` — mockup US-1
- `@.sdd/python/sdd_scripts/bench_run.py` — script d'agrégation
- `@.sdd/docs/benchmarks/README.md` — protocole général
- `@.sdd/docs/roi-baseline.md` — destination publication
- `@.sdd/docs/poc-roi-methodology.md` — méthodologie source
