...
> **MINOR + PATCH v7.x** (cf. `@.sdd/docs/VERSIONING.md` §3.2 + bannière haute).
...
- **C6** — invocation canonique `python .sdd/python/sdd_reverse_scripts/{x}.py`
...
- Parité helpers **bidirectionnelle** (`_parity_snapshots.json` hash aussi
  ...
  dossier parasite `.sdd/python/workspace/` supprimé) ;
...
- **MN2** : `.sdd/stacks/README.md` annoté "index non-stack" (clarifie comptage 34 vs 35 fichiers `*.md`).
...
**Réversibilité** : `git checkout main -- .claude/mcp.json .sdd/docs/MCP-SERVER.md`.
...
> **v7.0.0-alpha audit P0-doc 2026-06-05** — cette entrée historique décrivait un `git mv` de 6 stacks (`python-pytest`, `angular-jasmine`, `blazor-bunit`, `kotlin-mustache`, `blazor-server`, `kotlin-android`) vers `.sdd/stacks/_archive-v7.0.0/`. **Vérification factuelle 2026-06-05** : le dossier `_archive-v7.0.0/` n'existe pas, les 6 stacks sont toujours présents sous `.sdd/stacks/{qa,fullstack,mobiles}/` avec leur entête `Validation:` originale. Le sweep n'a jamais été commit. Entrée préservée pour transparence historique mais explicitement annulée pour éviter qu'un lecteur conclue à un état FS inexistant.
>
...
- `.sdd/python/sdd_mcp/` (13 modules, ~1957 LOC, ~600 LOC tests transitifs)
- `.sdd/python/tests/test_mcp_*.py` (8 fichiers, 1390 LOC)
- `.claude/mcp.json` (manifest client)
- `.sdd/docs/MCP-SERVER.md` (design doc)
...
`git checkout v6.10.4-LTS -- .sdd/python/sdd_mcp .claude/mcp.json`
(ou tag v7.0.0).
...
- Surface unique sous `.sdd/stacks/{cat}/` ; statut explicite par
...
- `.sdd/rules/library-and-stack.md` (mention blazor-server recalée)
- `.claude/agents/{dev-backend,dev-frontend,arch,arch-reviewer}.md` (status)
- `.sdd/docs/{architecture,validated-combos,scope-reduction-v7-ga}.md`
- `.sdd/python/sdd_admin/{validate_stack_md_headers,validate_libs_catalog,framework_smoke}.py`
  (retrait des filtres `_drafts` — devenus inutiles)
- `.sdd/stacks/README.md` (réécrit en README générique du catalogue)
...
- **`workspace/stack/stack.md.template`** → **`.sdd/templates/stack.md.template`**.
  ...
  `.sdd/templates/`. `workspace/` reste réservé aux **inputs
  utilisateur** (stack.md résolu, feats/, ui/), pas aux templates framework.
- Callers patchés :
  - `bootstrap.py:62` — `STACK_TEMPLATE` pointe vers `.sdd/templates/stack.md.template`
...
- **Fixture** : `.sdd/python/tests/fixtures/e2e-combo-c1/1-Minimal.md`
...
- **`dashboard`** (Haiku 4.5) supprimé. Remplacement déterministe :
  `python .sdd/python/sdd_scripts/index_adrs.py` (0 token, ~50 ms,
...
- **`security-reviewer` mode `threat-model`** supprimé. L'agent reste
  ...
  `.sdd/templates/threat-model.template.md` (STRIDE light, ~150 LOC).
...
10 stacks déplacés vers `.sdd/stacks/_drafts/` (non chargés par
...
> parente. Cf. `.sdd/stacks/_drafts/README.md`.
...
- **Templates** : `.sdd/templates/threat-model.template.md` (STRIDE
