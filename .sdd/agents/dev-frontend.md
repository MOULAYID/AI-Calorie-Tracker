...
3. **`.sdd/rules/quality.md`** — discipline tokens CSS. Source de
...
4. `.sdd/stacks/frontend/*.md`, `.sdd/stacks/ui/*.md`, et
   `.sdd/stacks/auth/*.md` (si `## Active Auth Specs` non vide) listés
...
|---|---|---|---|
| `back-front` | `web` | `.sdd/stacks/frontend/{stack-id}.md` | obligatoire si mockup HTML présent (`.sdd/stacks/ui/{stack-id}.md` de `## Active UI Specs`) |
| `back-front` | `mobile` | `.sdd/stacks/mobiles/{stack-id}.md` | 🟡 expérimental — chargeable mais aucun combo `mobile` validé bout-en-bout. UI DS peut être intégré au stack mobile (cf. §1.x du stack). |
| `back-front` | `null` | aucun frontend → exit silencieux (backend-only) | — |
| `fullstack` | `null` | `.sdd/stacks/fullstack/{stack-id}.md` | 🟡 expérimental — chargeable mais aucun combo `fullstack` validé bout-en-bout. Pour stabilité maximale, préférer `back-front`. |
...
Compléments à `@.sdd/rules/build-and-loop.md §7.4` (générique) :
...
- path: {chemin}
  ...
  # (autres champs : cf. @.sdd/rules/build-and-loop.md §7.4)
...
- Exit silencieux "backend-only US" : `@.sdd/rules/build-and-loop.md §7.3`
- AC UI coverage : `@.sdd/rules/build-and-loop.md §7.2` (AC-UI au lieu d'AC)
- Anti-derive plan : `@.sdd/rules/build-and-loop.md §7.5` + spécifique frontend : aucun
...
3. Helper métadonnées (déterministe, 0 token LLM) :
   ```bash
   python .sdd/python/sdd_scripts/compute_plan_metadata.py \
...
Invoquer :
```bash
python .sdd/python/sdd_scripts/validate_fidelity.py \
...
terminé. Pattern partagé — appliquer `@.sdd/rules/build-and-loop.md §6`
...
- Aucun composant hors mapping `.sdd/stacks/ui/{stack}.md §2/§7`
...
Applique `@.sdd/rules/output-protocol.md` (label `[DEV-FRONTEND]`, plage `66-78%`).
