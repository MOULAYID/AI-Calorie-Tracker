...
5. **Rapport consolidé** — plus aucun fichier écrit (SQLite = source unique).
   Rendu à la demande : `python .sdd/python/sdd_scripts/query_console_db.py
...
- Si vide ET `QAMode ≠ off` → ERROR :
  ...
  FIX: ajouter au moins un .sdd/stacks/qa/*.md dans workspace/stack/stack.md
  ```
...
edge-case ownership) : Read `@.sdd/rules/{nom}.md` à la demande.
...
```bash
python .sdd/python/sdd_scripts/quality_scan.py --feat-number {n}
```
...
```bash
python .sdd/python/sdd_scripts/parse_coverage.py --feat-number {n}
```
...
**Read on-demand `@.sdd/stacks/qa/mutation-testing.md §2-§5`**.
...
`@.sdd/stacks/qa/playwright.md §2-§5`**.
...
```bash
python .sdd/python/sdd_scripts/validate_acceptance.py
```
...
**Read on-demand si cas-limite** : `@.sdd/rules/quality.md`,
`@.sdd/rules/library-and-stack.md`.

---

## Chat Output Protocol

Applique `@.sdd/rules/output-protocol.md` (label `[QA]`, plage `58-66%` mode API Gate
