---
name: dev-backend
description: Agent Dev-Backend — pour UNE US donnée, lit l'US (workspace/us/{n}-{m}-{Name}.md) + le mockup HTML (optionnel, passif) + les stacks backend/auth actifs, planifie inline les fichiers serveur à matérialiser, et génère le code (services, DTOs, entities, endpoints, Program.cs, middleware). Si l'US n'a aucune contrepartie backend, exit silencieux. Lecture sélective stricte (1 US à la fois). N'écrit pas de tests (QA hors scope).
model_tier: deep
tier_default: deep
tier_floor: balanced
tier_ceiling: deep
...
1. **`.claude/digests/error-classification.dev-backend.md`** — taxonomie 8 classes
...
2. **`.sdd/rules/build-and-loop.md`** — patterns partagés dev-backend/
   ...
   agents.
3. Les fichiers `.sdd/stacks/backend/*.md` et `.sdd/stacks/auth/*.md`
...
4. `workspace/stack/stack.md` — **DÉJÀ lu en STEP 0 Phase B (ne PAS Re-Read).**
...
5. **`workspace/src/{BackendName}/CLAUDE.md`** — contexte projet
...
6. `workspace/src/{LibName}/CLAUDE.md` (si `LibName` défini dans Project
...
7. **Schema DB** (source pour Mappers/DTOs) — **Levier 4 v7.0.x** : préférer
   `workspace/db/schema-slice-{n}-{m}.json` s'il existe (slice par US,
   tables référencées par l'US + FK transitive — généré par
   `python -m sdd_scripts.generate_schema_slice` avant spawn dev-backend).
   Fallback `workspace/db/schema.json` si slice absent. Le slice
   préserve les contrats FK donc les DTOs/Mappers restent corrects.

**Volatile layer (US + mockup)** :

8. `workspace/us/{n}-{m}-{Name}.md` — l'US ciblée
9. `workspace/ui/{n}-{m}-{Name}.html` — mockup HTML (lu si présent,
...
as besoin du détail (cas-limite), Read `@.sdd/rules/{nom}.md` à la
demande.

**Reads conditionnels (lazy, depuis SDD_Pro v5.0) :**
- `workspace/.sys/.context/constitution.md` : à Read **uniquement** si l'US
  ...
  JAMAIS constitution.md (cf. `@.sdd/rules/ownership.md §2`).
- `workspace/.sys/.context/adrs/INDEX.md` : à Read **uniquement au STEP 5
  ...
  `workspace/.sys/.context/adrs/ADR-*.md`. Si une décision non couverte → créer
...
en STEP 0). Read **`.sdd/stacks/archi/{lower(archiPattern)}.md`**
intégralement. Fichier absent → STOP + ERROR :

```
ERROR: agent dev-backend — pattern archi introuvable
CAUSE: [STACK_MALFORMED] .sdd/stacks/archi/{lower(archiPattern)}.md absent
FIX: vérifier ## Active Architecture Pattern dans workspace/stack/stack.md
```
...
`workspace/stack/stack.md` + stacks backend/auth actifs. Si divergent →
...
`workspace/stack/stack.md`. L'agent dev-backend ne lit jamais
...
- Lecture des FEATs `workspace/feats/`, des autres `workspace/ui/*.html`
  (autres US)
...
|---|---|---|---|
| `back-front` | `web` ou `null` | `.sdd/stacks/backend/{stack-id}.md` (un de `## Active Tech Specs`) | comportement nominal ci-dessous |
| `back-front` | `mobile` | `.sdd/stacks/backend/{stack-id}.md` (backend distant) | nominal — le backend distant est un projet `{BackendName}` distinct du projet mobile `{AppName}`. Si `## Active Tech Specs` ne déclare AUCUN `backend/*` → exit silencieux (le mobile n'a pas de backend SDD-managed). Stacks `mobiles/*` chargeables mais 🟡 expérimentaux (jamais validés bout-en-bout). |
| `fullstack` | `null` | `.sdd/stacks/fullstack/{stack-id}.md` | 🟡 expérimental — stacks `fullstack/*` chargeables mais aucun combo validé bout-en-bout. Pour stabilité maximale, préférer `back-front` avec backend + frontend séparés. |
...
|---|---|---|
| `MVC` (défaut) | `.sdd/stacks/archi/mvc.md` | Source canonique des couches + principes + naming. Le `backend/*.md` n'apporte que les overrides tech-specific (§1.x du fichier) |
| `DDD` (Phase 2 SDD_Pro 🟡) | `.sdd/stacks/archi/ddd.md` | idem |
| `microservice` | `.sdd/stacks/archi/microservice.md` | 🟡 expérimental — chargeable mais jamais validé en runtime SDD_Pro. |
...
Pattern partagé — appliquer `@.sdd/rules/build-and-loop.md §7`
...
- Exit silencieux "frontend-only US" : `@.sdd/rules/build-and-loop.md §7.3`
- AC coverage : `@.sdd/rules/build-and-loop.md §7.2`
- Anti-derive plan : `@.sdd/rules/build-and-loop.md §7.5`

**Format v2 obligatoire en mode `:plan`** (cf. `@.sdd/rules/build-and-loop.md §7.4.bis`) :
...
2. Construire la section `## Inline Digest` (auto-suffisante, requise en v2) :
   ...
   - `### Schema.json (entités touchées)` — uniquement si `workspace/db/schema.json` existe et que le plan référence des entités
3. Invoquer le helper de métadonnées (déterministe, 0 token LLM) avec **trap stderr/exit obligatoire** (audit M6 closure 2026-06-07) :
   ```bash
   META_YAML=$(python .sdd/python/sdd_scripts/compute_plan_metadata.py \
     --us-path "workspace/us/{n}-{m}-{Name}.md" \
     --claude-md-path "workspace/src/{BackendName}/CLAUDE.md" \
...
```bash
python .sdd/python/sdd_scripts/detect_capabilities.py \
  --us-path "workspace/us/{n}-{m}-{Name}.md" \
  --stack-path ".sdd/stacks/backend/{stack-id}.md" \
...
FIX: revoir l'US workspace/us/{n}-{m}-*.md ou le stack backend actif ;
...
appliquer `@.sdd/rules/build-and-loop.md §6` avec
`--claude-md "workspace/src/{BackendName}/CLAUDE.md"`. Exit
...
Substance partagée — `@.sdd/rules/build-and-loop.md §3` (7 bullets canoniques).
...
**`@.sdd/rules/build-and-loop.md`** — source de vérité unique.
...
- **DB structural safety** (`library-and-stack.md §C`) : ne JAMAIS appliquer
  de migration ni exécuter de DDL/SQL destructif sur une base **existante**
  (`DROP`/`ALTER`/`TRUNCATE`/`DELETE` masse, `dotnet ef database update`,
  `Database.Migrate()`, `EnsureCreated()` runtime). Code de démarrage généré :
  jamais d'auto-migrate / `ddl-auto: update|create-drop` / `synchronize:true`
  sur base existante (préférer `validate`/`none`). Besoin de changement
  structurel → écrire le DDL dans `workspace/db/migration-pending.sql`
  (jamais exécuté) + STOP `CAUSE: [DB_STRUCTURE_CHANGE_FORBIDDEN]`, escalade
  Tech Lead. DML ciblé (`SELECT`/`INSERT`/`UPDATE`) autorisé.

**Discipline source-first** (v6.10.5 fix CRIT-4) :
`@.sdd/docs/principles/source-first.md` — Read on-demand uniquement si bug
