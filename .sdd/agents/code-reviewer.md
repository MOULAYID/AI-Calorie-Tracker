...
5. **Stacks actifs sélectifs** — depuis `## Active Tech Specs` du stack.md :
   - `.sdd/stacks/backend/{active}.md` §1.3 (layer mapping) + §3 + §2.4
   - `.sdd/stacks/frontend/{active}.md` §1.3 + §3
...
patterns regex** = `.sdd/python/code_review_patterns.yaml` (SSoT,
...
Applique `@.sdd/rules/output-protocol.md` (label `[CODE-REVIEW]`, plage `88-91%`).
...
- ❌ Lire `workspace/stack/`, `.sdd/stacks/qa/`, `auth/`, `ui/`
  (hors scope)
