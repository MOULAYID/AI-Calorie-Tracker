...
> §2.4 (Librairies) régénérée depuis `dotnet-minimalapi.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id dotnet-minimalapi`).
...
> `.sdd/stacks/archi/{ArchiPattern}.md` (defaut `MVC` si `## Active Architecture Pattern`
...
> Source de verite : `.sdd/stacks/backend/dotnet-minimalapi.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id dotnet-minimalapi`.
...
DRIVER_MODULE=$(jq -r ".dbDrivers.\"$DB_TYPE\".module" .sdd/stacks/backend/dotnet-minimalapi.libs.json)
DRIVER_VERSION_REF=$(jq -r ".dbDrivers.\"$DB_TYPE\".ref" .sdd/stacks/backend/dotnet-minimalapi.libs.json)
DRIVER_VERSION=$(jq -r ".versions.\"$DRIVER_VERSION_REF\"" .sdd/stacks/backend/dotnet-minimalapi.libs.json)
...
## 7. CORS
Conforme a `.sdd/rules/library-and-stack.md §2.1`. Policy `Spa` avec origins **explicites**
...
Cf. `.sdd/stacks/frontend/react.md §5` pour le grep côté frontend.
...
stack (voir `.sdd/rules/library-and-stack.md`). Toute librairie
