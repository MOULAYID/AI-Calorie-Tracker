...
> §2.4 (Librairies) régénérée depuis `kotlin-spring-boot.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id kotlin-spring-boot`).
...
> Source de verite : `.sdd/stacks/backend/kotlin-spring-boot.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id kotlin-spring-boot`.
