...
> §2.4 (Librairies) régénérée depuis `python-fastapi.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id python-fastapi`).
...
> Source de verite : `.sdd/stacks/backend/python-fastapi.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id python-fastapi`.
...
Conforme à `.sdd/rules/library-and-stack.md §2.3`. Origins **explicites** lus depuis env
