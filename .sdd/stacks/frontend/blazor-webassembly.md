...
> §2.4 (Librairies) régénérée depuis `blazor-webassembly.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id blazor-webassembly`).
...
ACTIVE_UI=$(grep -E '^- \.sdd/stacks/ui/' workspace/tech/stack.md 2>/dev/null | grep -v '^#' || true)
...
ACTIVE_AUTH=$(grep -E '^- \.sdd/stacks/auth/' workspace/stack/stack.md 2>/dev/null | grep -v '^#' || true)
...
> Source de verite : `.sdd/stacks/frontend/blazor-webassembly.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id blazor-webassembly`.
