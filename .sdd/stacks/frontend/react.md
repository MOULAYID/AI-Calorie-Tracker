...
> §2.4 (Librairies) régénérée depuis `react.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id react`).
...
> Source de verite : `.sdd/stacks/frontend/react.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id react`.
...
`dotnet-minimalapi` (cf. `.sdd/stacks/backend/dotnet-minimalapi.md §URLs de
développement`) :
...
1. **Tokens globaux** (`src/index.css`) — variables CSS shadcn
   ...
   `shadcn init`. Surcharge possible via `.sdd/rules/quality.md §3`
