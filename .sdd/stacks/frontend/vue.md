...
> §2.4 (Librairies) régénérée depuis `vue.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id vue`).
...
> Source de verite : `.sdd/stacks/frontend/vue.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id vue`.
...
Conformes à `.sdd/rules/library-and-stack.md §0 (Partie A — Pattern B
secrets/config)`. Les variables `VITE_*` sont **publiquement exposées**
côté navigateur — ne jamais y mettre de secret.

### 4.2 CORS

Le backend doit avoir une policy CORS `DevOpen` (cf. `.sdd/rules/library-and-stack.md`).
...
- **UI design system** : selon `## Active UI Specs` (Vuetify, ...)
  - Si Vuetify actif : voir `.sdd/stacks/ui/vuetify.md`
