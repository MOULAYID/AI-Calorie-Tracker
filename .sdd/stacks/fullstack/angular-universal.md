...
> **Patterns Angular partages** (standalone, signals, HttpClient, control flow `@if`/`@for`) : voir `.sdd/stacks/frontend/angular.md §1.1-§1.4` (integralement applicable cote client). Specificite SSR : ajouter le module `provideServerRendering()` dans `app.config.server.ts`.
...
**Patterns partages** : voir `.sdd/stacks/frontend/angular.md §1.4` (SOLID, Clean Code, anti-patterns deja documentes — integralement applicable).

---

## 1.5 Couches persistantes

Patterns reconnus : `Entity`, `Entities`, `Repository`, `Repositories`, `Migration`, `Migrations`. Pattern Prisma identique a `.sdd/stacks/backend/node-express.md §8` (le serveur SSR est Express). Mode file-based : pattern `node-react.md §6.2`.
...
Si CSR-only suffit → preferer `.sdd/stacks/frontend/angular.md` simple. Ce stack n'a de sens que si SSR est requise.
...
> Source de verite : `.sdd/stacks/fullstack/angular-universal.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id angular-universal`.
