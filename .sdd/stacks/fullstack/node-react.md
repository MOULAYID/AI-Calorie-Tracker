...
> Source de verite : `.sdd/stacks/fullstack/node-react.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id node-react`.
...
| `fullstack-node-react` + `auth-local` + `qa-node-vitest` + `SqlServer` (Prisma) | 🟢 reference | Demo POC 2026-05-26 (Prisma auto-activé via `## Active Database`) |
