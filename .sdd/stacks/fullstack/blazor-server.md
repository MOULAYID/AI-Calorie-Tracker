...
- **Infos utilisateur** : pour tout acces aux donnees de l'utilisateur courant (identifiant, nom, email, groupes), passer par le service scoped `UserAd` (voir `.sdd/stacks/auth/azure-ad.md` §5.3). Pas d'acces direct a `AuthenticationStateProvider` dans le code-behind hors du point de chargement initial ; pas de lecture ad-hoc de claims eparpillee dans les composants.
...
> Source de verite : `.sdd/stacks/fullstack/blazor-server.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id blazor-server`.
...
- Lecture directe de claims (`authState.User.FindFirst(...)`) eparpillee dans plusieurs composants au lieu de passer par le service scoped `UserAd` (voir `.sdd/stacks/auth/azure-ad.md` §5.3)
...
courant, passer par `UserAd` (defini dans `.sdd/stacks/auth/azure-ad.md` §5.3). Le
