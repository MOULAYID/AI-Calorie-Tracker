...
> §2.4 (Librairies) régénérée depuis `radzen-blazor.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id radzen-blazor`).
...
> Source de verite : `.sdd/stacks/ui/radzen-blazor.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id radzen-blazor`.
...
L'init script du stack frontend Blazor WASM (`.sdd/stacks/frontend/blazor-webassembly.md`
STEP 3c) DOIT injecter ces deux lignes au scaffold initial quand
`.sdd/stacks/ui/radzen-blazor.md` est dans `## Active UI Specs`.
