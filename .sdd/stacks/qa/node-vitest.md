...
> §2.4 (Librairies) régénérée depuis `node-vitest.libs.json` — ne pas éditer manuellement (`python .sdd/python/sdd_admin/sync_stack_md.py --stack-id node-vitest`).
...
> Source de verite : `.sdd/stacks/qa/node-vitest.libs.json`. Ne pas editer cette section manuellement -- utiliser `.sdd/python/sdd_admin/sync_stack_md.py --stack-id node-vitest`.
