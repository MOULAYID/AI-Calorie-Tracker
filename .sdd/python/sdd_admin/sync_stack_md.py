...
def build_libs_table(cat: dict[str, Any], stack_id: str) -> str:
    ...
    sb.append("")
    sb.append(
        f"> Source de verite : `.sdd/stacks/{cat.get('category')}/{stack_id}.libs.json`. "
        f"Ne pas editer cette section manuellement -- utiliser "
        f"`.sdd/python/sdd_admin/sync_stack_md.py --stack-id {stack_id}`."
    )
    sb.append("")
...
def main() -> int:
    ...
    for f in (stacks_dir(root)).rglob(f"{args.stack_id}.libs.json"):
        catalog_path = f
        break
