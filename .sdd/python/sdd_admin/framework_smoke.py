...
def _fingerprint(claude_root: Path) -> str:
    ...
    """
    # Bi-racine 2026-07-25 : agents/commands restent dans .claude/ (façade générée),
    # rules/stacks/templates/docs/python vivent dans .sdd/ (foyer neutre SSoT).
    ...
    roots = (
        ...
        sdd_root / "docs",
        sdd_root / "templates",
        sdd_root / "python" / "sdd_scripts",
        sdd_root / "python" / "sdd_admin",
        sdd_root / "python" / "sdd_hooks",
        sdd_root / "stacks",
    )
...
# `.sdd/docs/rubrics/complexity-router-scoring.md`. Living in agents/ was
...
EXPECTED_RULES = (
    ...
    # `.sdd/docs/principles/` (Tech Lead discipline / mono-agent po,
...
def _check_bom_files(claude_root: Path, checks: "Checks") -> None:
    ...
    from sdd_lib.paths import sdd_home as _sdd_home
    sdd_root = _sdd_home(claude_root.parent)
    strip_bom_path = sdd_root / "python" / "sdd_admin" / "strip_bom.py"
    ...
    try:
        ...
        if res.returncode == 0 and n_with_bom == 0:
            ...
            checks.add("framework-bom-check", "WARN",
                       ...
                       f"run `python .sdd/python/sdd_admin/strip_bom.py` to fix")
        else:
            checks.add("framework-bom-check", "WARN",
...
def _check_telemetry_health(claude_root: Path, checks: "Checks") -> None:
    ...
    from sdd_lib.paths import sdd_home as _sdd_home
    sdd_root = _sdd_home(claude_root.parent)
    telemetry_path = sdd_root / "python" / "sdd_admin" / "verify_telemetry_health.py"
    ...
    try:
        ...
        if verdict in ("CLEAN", "ABSENT", "PRISTINE"):
            ...
            checks.add("telemetry-health", "WARN",
                       ...
                       "`python .sdd/python/sdd_admin/verify_telemetry_health.py`")
    ...
    try:
        ...
        if res.returncode == 0 and errs == 0:
            ...
            checks.add("project-config-schema", "WARN",
                       ...
                       f"`python .sdd/python/sdd_scripts/"
...
def _check_stack_md_headers(claude_root: Path, checks: "Checks") -> None:
    ...
    from sdd_lib.paths import sdd_home as _sdd_home
    sdd_root = _sdd_home(claude_root.parent)
    headers_script = sdd_root / "python" / "sdd_admin" / "validate_stack_md_headers.py"
...
def _check_adr_naming(claude_root: Path, checks: "Checks") -> None:
    ...
    from sdd_lib.paths import sdd_home as _sdd_home
    sdd_root = _sdd_home(claude_root.parent)
    script = sdd_root / "python" / "sdd_admin" / "validate_adr_naming.py"
...
def main() -> int:
    ...
    root = repo_root()
    # Bi-racine 2026-07-25 : les couches sont sous `.sdd/` (foyer neutre) ou
    # sous `.claude/` (façade générée). Les helpers résolvent la racine réelle.
    from sdd_lib.paths import (
        agents_dir as _agents_dir,
        commands_dir as _commands_dir,
        rules_dir as _rules_dir,
        stacks_dir as _stacks_dir,
        templates_dir as _templates_dir,
        docs_dir as _docs_dir,
        python_dir as _python_dir,
        sdd_home,
    )
    claude_root = root / ".claude"  # generated facade (agents, commands, CLAUDE.md, digests)
    sdd_root = sdd_home(root)  # SSoT for rules, stacks, templates, docs, python, skills
    ...
    rubrics_dir = sdd_root / "docs" / "rubrics"
    ...
    principles_dir = sdd_root / "docs" / "principles"
    ...
    py_dir = sdd_root / "python" / "sdd_scripts"
    ...
    admin_dir = sdd_root / "python" / "sdd_admin"
    ...
    py_lock = sdd_root / "python" / "sdd_scripts" / "gate_decide.py"
