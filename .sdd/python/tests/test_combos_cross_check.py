"""Cross-check tests for `.sdd/templates/combos.json` SSoT.

v7.0.0-alpha (audit CRIT-6, 2026-06-04) — kills drift between :
  - `.sdd/templates/combos.json`               (machine SSoT)
  - `.sdd/stacks/{cat}/{stack-id}.md`          (human SSoT — `Validation:` header)
...
# `db` and `archi` are config-level (not standalone `.sdd/stacks/db/` dir
...
class TestStackMdCrossCheck(unittest.TestCase):
    """For each category declared in combos.json, each stack_id MUST :
      (a) have a matching `.sdd/stacks/{cat}/{stack_id}.md` file ;
    ...
    def test_componentlevels_match_md_headers(self):
        ...
        for category, stacks in levels.items():
            if category not in _MD_BACKED_CATEGORIES:
                continue
            from sdd_lib.paths import stacks_dir as _sd
            stacks_dir = _sd(REPO_ROOT) / category
