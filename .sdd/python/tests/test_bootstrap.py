...
# Bi-racine 2026-07-25 : stacks migrées vers .sdd/stacks/.
from sdd_lib.paths import stacks_dir as _stacks_dir  # noqa: E402
_STACKS = _stacks_dir(_REPO)
...
class TestCombos(unittest.TestCase):
    ...
    def test_combo_stacks_exist_on_disk(self):
        """Each declared stack must have a corresponding .md in .sdd/stacks/."""
        for name, combo in bootstrap.COMBOS.items():
            backend_md = _STACKS /"backend" / f"{combo['backend']}.md"
            frontend_md = _STACKS /"frontend" / f"{combo['frontend']}.md"
            ui_md = _STACKS /"ui" / f"{combo['ui']}.md"
            archi_md = _STACKS /"archi" / f"{combo['archi']}.md"
            for path in (backend_md, frontend_md, ui_md, archi_md):
                self.assertTrue(path.is_file(),
                                f"combo {name} references missing stack {path}")
            for qa in combo["qa"]:
                qa_md = _STACKS /"qa" / f"{qa}.md"
...
class TestRenderStackMd(unittest.TestCase):
    ...
    def test_substitutes_archi(self):
        ...
        self.assertIn(".sdd/stacks/archi/ddd.md", out)
    ...
    def test_substitutes_active_tech_specs(self):
        ...
        self.assertIn(".sdd/stacks/backend/kotlin-spring-boot.md", out)
        self.assertIn(".sdd/stacks/frontend/vue.md", out)

    def test_substitutes_qa_list(self):
        out = self._render(qa=["dotnet-xunit", "node-vitest"])
        self.assertIn(".sdd/stacks/qa/dotnet-xunit.md", out)
        self.assertIn(".sdd/stacks/qa/node-vitest.md", out)
    ...
    def test_azure_ad_block_commented_keys(self):
        ...
        self.assertIn(".sdd/stacks/auth/azure-ad.md", out)
    ...
    def test_auth_local_block_generates_real_jwt_secret(self):
        ...
        self.assertIn(".sdd/stacks/auth/auth-local.md", out)
    ...
    def test_auth_none_does_not_activate_any_profile(self):
        ...
        self.assertNotIn(".sdd/stacks/auth/azure-ad.md\n", out)
        self.assertNotIn(".sdd/stacks/auth/auth-local.md\n", out)
