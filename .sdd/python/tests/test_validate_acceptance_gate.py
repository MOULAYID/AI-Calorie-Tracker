...
gate (test/lint/build/coverage) on every project under workspace/src/.
...
Baseline pinned :
  ...
  - No projects in workspace/src/ → DENY (no acceptance.json) — use bypass
"""
...
class TestValidateAcceptanceGate(unittest.TestCase):
    def test_empty_stdin_denies(self):
        ...
        # Isolate the project root to an empty temp dir — otherwise the hook
        # resolves the real (sibling) project workspace, which may already
        # hold an acceptance.json (split-layout resolution, audit 2026-07-06).
        with TemporaryDirectory() as td:
            proj = Path(td) / "proj"
            proj.mkdir()
            r = _run_hook({}, {"CLAUDE_PROJECT_DIR": str(proj)})
    ...
    def test_no_projects_under_src_denies(self):
        """Empty workspace/src/ + no acceptance.json → DENY (CRIT-4)."""
