...
be parsed by a real Python `argparse` somewhere in `.sdd/python/`, OR
...
class TestCliFlagsDeclared(unittest.TestCase):
    ...
    def test_command_flags_have_implementation_or_annotation(self):
        ...
        python_dir = _repo_root() / ".sdd" / "python"
        python_files = list(python_dir.rglob("*.py"))
        self.assertGreater(len(python_files), 50, "expected many Python files in .sdd/python/")
