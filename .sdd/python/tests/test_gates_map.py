...
from sdd_lib.paths import repo_root, docs_dir, python_dir, rules_dir, commands_dir

pytestmark = pytest.mark.smoke

_PY_ENFORCER_RE = re.compile(r"`(sdd_(?:hooks|scripts|admin)/[a-z0-9_]+\.py)`")
# Bi-racine 2026-07-25 : docs+rules migrés vers .sdd/, commands restent
# dans .claude/. Le pattern accepte les deux racines.
_MD_REF_RE = re.compile(r"`@?\.(?:claude|sdd)/(rules|commands|docs)/([a-z0-9-]+\.md)")


def _gates_map_text() -> str:
    p = docs_dir(repo_root()) / "gates-map.md"
    assert p.is_file(), f"gates-map.md absent — registre des gates supprimé ? ({p})"
    return p.read_text(encoding="utf-8", errors="replace")


def _resolve_md_ref(root: Path, sub: str, name: str) -> Path:
    """Resolve a `.claude/{sub}/{name}` or `.sdd/{sub}/{name}` reference to actual dir."""
    if sub == "rules":
        return rules_dir(root) / name
    if sub == "commands":
        return commands_dir(root) / name
    if sub == "docs":
        return docs_dir(root) / name
    return root / ".claude" / sub / name  # fallback (shouldn't happen given regex)


def test_cited_python_enforcers_exist():
    root = python_dir(repo_root())
...
def test_cited_markdown_refs_exist():
    ...
    for sub, name in set(_MD_REF_RE.findall(text)):
        if not _resolve_md_ref(root, sub, name).is_file():
