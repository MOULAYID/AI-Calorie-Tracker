...
def _read_ownership_md() -> str:
    """Read ownership.md (source of truth, post-sweep v7.0.0).

    Bi-racine 2026-07-25 : rules migrées vers `.sdd/rules/` (Phase 1).
    `rules_dir()` retourne `.sdd/rules/` si présent, sinon `.claude/rules/`.
    """
    from sdd_lib.paths import rules_dir
    p = rules_dir() / "ownership.md"
...
class TestOwnershipMatrixSync(unittest.TestCase):
    ...
    def test_no_retired_v7_agent_in_matrix(self) -> None:
        ...
        for retired in RETIRED_V7_AGENTS:
            self.assertNotIn(
                ...
                f"Remove from .sdd/python/sdd_hooks/audit_file_ownership.py.",
            )
    ...
    def test_matrix_agents_are_v7_active(self) -> None:
        ...
        self.assertEqual(
            ...
            f"from .sdd/python/sdd_hooks/audit_file_ownership.py.",
        )
