...
Run from repo root (any of):
    python -m unittest discover -s .sdd/python/tests -t .claude/python -v
    python -m pytest .sdd/python/tests/
    python .sdd/python/tests/test_<name>.py
...
This __init__ also adds `.sdd/python/` to sys.path so that tests
