...
def _run(monkeypatch, tmp_path: Path, skill: str, args: str,
    ...
    # Simpler: the hook builds `root/.sdd/python/sdd_reverse_scripts/...`;
    ...
    dest = tmp_path / ".sdd" / "python" / "sdd_reverse_scripts"
    dest.mkdir(parents=True, exist_ok=True)
    (tmp_path / ".sdd" / "python" / "sdd_reverse").mkdir(parents=True, exist_ok=True)
    ...
    if not (dest / "check_reverse_feat_for_full.py").exists():
        shutil.copytree(_PY_ROOT / "sdd_reverse",
                        tmp_path / ".sdd" / "python" / "sdd_reverse",
                        dirs_exist_ok=True)
