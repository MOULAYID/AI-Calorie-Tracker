...
# Bi-racine 2026-07-25 (MIGRATION-PLAN Phase 1) : accepte les stacks
# référencés soit sous `.claude/stacks/` (legacy) soit sous `.sdd/stacks/`
# (foyer neutre). Cf. `sdd_lib.project_config._ACTIVE_STACK_RE`.
ACTIVE_PATTERN = re.compile(
    r"^\s+-\s+(?:\.claude|\.sdd)/stacks/(backend|frontend|fullstack|mobiles|archi|ui|qa|auth)/([\w-]+)\.md\s*$",
    re.MULTILINE,
)
