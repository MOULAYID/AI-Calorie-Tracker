...
of precedence):
    ...
    3. Repo-root walk : ascend from CWD looking for `.sdd/python/sdd_reverse/`
...
def _candidate_from_cwd_walk() -> Path | None:
    """Walk up from CWD looking for `.sdd/python/sdd_reverse/`."""
    cwd = Path.cwd().resolve()
    for parent in (cwd, *cwd.parents):
        candidate = parent / ".sdd" / "python" / "sdd_reverse"
        if candidate.is_dir():
            return candidate
    return None
...
def sdd_reverse_dir() -> Path:
    ...
    Resolution order:
        ...
        3. Walk up from CWD looking for .sdd/python/sdd_reverse/
...
def repo_root() -> Path:
    ...
    `workspace/` trees when invoked from `.sdd/python/`. All workspace
