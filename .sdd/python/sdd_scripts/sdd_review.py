...
def _auto_ingest_orphan_jsons(feat_n: int) -> list[str]:
    ...
    for fname, type_arg, table in candidates:
        ...
        # repo root — `.sdd/python/sdd_scripts` is not on sys.path for
        # subprocess by default).
        ingest_script = root / ".sdd" / "python" / "sdd_scripts" / "ingest_agent_report.py"
        try:
            result = subprocess.run(
                ...
                env={**os.environ, "PYTHONPATH": str(root / ".sdd" / "python")},
            )
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"   report (on-demand): python .sdd/python/sdd_scripts/"
