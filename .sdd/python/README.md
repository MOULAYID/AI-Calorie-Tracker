...
continuent à fonctionner avec les anciens callers (`python .sdd/python/sdd_scripts/X.py`)
...
echo "hooks:   $(ls .sdd/python/sdd_hooks/*.py | grep -v __init__ | wc -l)"
echo "admin:   $(ls .sdd/python/sdd_admin/*.py | grep -v __init__ | wc -l)"
echo "lib:     $(find .sdd/python/sdd_lib -name '*.py' | grep -v __init__ | wc -l)"
echo "tests:   $(ls .sdd/python/tests/test_*.py | wc -l)"
```
...
```bash
# Suite complète
python -m pytest .sdd/python/tests/ -v

# Smoke uniquement (sous-ensemble enforcement load-bearing)
python -m pytest .sdd/python/tests/ -m smoke
...
stacks (`.sdd/stacks/**/*.md`) et hooks (`.claude/settings.json`)
référencent l'invocation Python canonique :

```bash
python .sdd/python/sdd_scripts/context_budget.py --agent po --feat-number {n}
python .sdd/python/sdd_admin/sync_stack_md.py --stack-id react
python .sdd/python/sdd_scripts/validate_plan.py --plan-path {path} --us-path {path} --json
```
...
- `pyproject.toml` — config pytest + ruff + mypy + coverage
- `.sdd/rules/error-classification.md §1.4` — taxonomie `[BUILD_*]` exit codes
