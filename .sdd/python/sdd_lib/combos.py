"""SSoT loader for `.sdd/templates/combos.json` — validated combos + per-component levels.
...
`.sdd/stacks/{cat}/{stack-id}.md` remains the **human-readable
...
def load_combos(root: Path | None = None) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            f"Restore from git or reinstall .sdd/templates/."
        )
