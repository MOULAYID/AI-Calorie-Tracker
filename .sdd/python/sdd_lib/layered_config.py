...
from sdd_lib.paths import templates_dir, repo_root
...
def _load_schema_known_keys(root: Path | None = None) -> set[str]:
    ...
    schema_path = templates_dir(root) / "project-config.schema.json"
...
def _warn_unknown_keys(effective: dict[str, Any], root: Path | None = None) -> None:
    ...
    if strict:
        raise ConfigError(
            ...
            fix="Register the key in .sdd/templates/project-config.schema.json#properties, or remove the typo from stack.md ## Project Config",
        )
...
def dump_effective_config(
    ...
    """
    from sdd_lib.paths import templates_dir, iso_now
