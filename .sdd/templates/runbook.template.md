...
> Source : SDD_Pro `.sdd/templates/runbook.template.md`
...
**Downstream consumers** : {liste, ex. frontend DemoFront, mobile app, partenaires API}
...
2. Check les logs `journalctl -u demoapp-back` (chercher `JWT_SIGNING_KEY` ou `AUTH_LOCAL`)
...
`.sdd/templates/postmortem.template.md` dans **48h max** (cf.
...
> `.sdd/templates/runbook.template.md` + complétion humaine.
